...
function estadoBadge(estado: EstadoEstudo, t: (key: string) => string) {
  const map: Record<string, { label: string; bg: string; fg: string }> = {
    UPLOADED:           { label: t('dashboard.statusUploaded'),   bg: 'var(--scolio-neutral-surface)',    fg: 'var(--scolio-neutral-gray)' },
    PROCESSING:         { label: t('dashboard.statusProcessing'), bg: 'var(--scolio-light-blue-surface)', fg: 'var(--scolio-primary-blue)' },
    PENDING_VALIDATION: { label: t('dashboard.statusReady'),      bg: 'var(--scolio-warning-surface)',    fg: 'var(--scolio-warning-amber)' },
    VALIDATED:          { label: t('dashboard.statusValidated'),  bg: 'var(--scolio-success-surface)',    fg: 'var(--scolio-success-green)' },
    ARCHIVED:           { label: t('dashboard.statusArchived'),   bg: 'var(--scolio-neutral-surface)',    fg: 'var(--scolio-neutral-gray)' },
  };
...
export default function TecnicoDashboardScreen() {
  ...
  const kpis = metricas
    ? [
        { icon: Upload, color: 'var(--scolio-success-green)', bg: 'var(--scolio-success-surface)', label: t('dashboard.uploadedToday'), value: String(metricas.carregadosHoje) },
        { icon: Loader2, color: 'var(--scolio-primary-blue)', bg: 'var(--scolio-light-blue-surface)', label: t('dashboard.processingAI'), value: String(metricas.emProcessamento) },
        { icon: CheckCircle2, color: 'var(--scolio-warning-amber)', bg: 'var(--scolio-warning-surface)', label: t('dashboard.readyForValidation'), value: String(metricas.prontoValidacao) },
        { icon: Archive, color: 'var(--scolio-neutral-gray)', bg: 'var(--scolio-neutral-surface)', label: t('dashboard.archived'), value: String(metricas.arquivados) },
      ]
    : [];
  ...
  return (
    <div className="p-8 space-y-6 overflow-auto h-full">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-[var(--scolio-text-primary)]">{t('dashboard.goodMorning', { name: primeiroNome })}</h1>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 bg-white rounded-[var(--radius-card)] shadow-sm border border-[var(--scolio-border-light)] overflow-hidden">
          <div className="p-6 flex items-center justify-between">
            <h3 className="text-[var(--scolio-text-primary)]">{t('dashboard.workQueue')}</h3>
            <button
          ...
          <table className="w-full">
            <thead>
              <tr className="border-y border-[var(--scolio-border-light)] bg-[var(--scolio-page-surface)]">
                {[t('dashboard.tablePatient'), t('dashboard.tableExamDate'), t('dashboard.tableStatus'), t('dashboard.tableSubmitted')].map((h) => (
        ...
        <div className="bg-white rounded-[var(--radius-card)] shadow-sm border border-[var(--scolio-border-light)] p-6">
          <h3 className="text-[var(--scolio-text-primary)] mb-4">{t('dashboard.recentActivity')}</h3>
          {aCarregar ? (
